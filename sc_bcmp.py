"""
SINGLE CLASS BCMP NETWORK LIBRARY
...
[ABGdMT06] numbers the stations from 1 on; here we should start at 0 (conforming to the Python, and C, standard).
...
class BCMP(object):
    def __init__(self, name, e, m, mu, jobs, use_logs=True, iter=True):
        ...
        :param e: visit ratios: avg number of visits to each station by a job.
                  It is the solution of e = eR with r_ij = prob a job goes from station i to station j, setting e_1 = 1 to get a unique solution.
    ...
    def beta(self, i, k):
        return self._log_beta(i, k) if self.use_logs else self._beta(i, k)
